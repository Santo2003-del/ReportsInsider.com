      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 py-1 px-3 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold tracking-widest uppercase mb-5"
          >
            Our Strengths
          </motion.div>
          <motion.h2
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {REASONS.map((reason, index) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              className="bg-white/[0.04] backdrop-blur-sm rounded-xl p-5 flex flex-col items-center justify-center text-center group hover:bg-white/[0.08] border border-white/5 hover:border-blue-500/30 shiny-card transition-all duration-300"
            >
              <div className="w-11 h-11 rounded-lg bg-blue-500/10 border border-blue-500/15 flex items-center justify-center mb-3 group-hover:bg-blue-600 group-hover:border-blue-500 transition-all duration-300">
                <reason.icon className="w-5 h-5 text-blue-400 group-hover:text-white transition-colors duration-300" />
              </div>
              <h3 className="text-sm font-semibold text-slate-200 group-hover:text-white transition-colors mb-1">
                {reason.title}
              </h3>
              <p className="text-xs text-slate-500 leading-relaxed hidden sm:block">{reason.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
